...
import de.interoberlin.poisondartfrog.model.wunderbar.tasks.ScanWunderbarTask;
...
import de.interoberlin.poisondartfrog.view.dialogs.WunderbarScanResultsDialog;
import io.relayr.android.RelayrSdk;
import io.relayr.android.ble.BleDevice;
import io.relayr.java.ble.BleDeviceType;
import io.relayr.java.model.action.Reading;

public class WunderbarDevicesActivity extends AppCompatActivity implements ScanWunderbarTask.OnCompleteListener, SubscribeWunderbarTask.OnCompleteListener, WunderbarScanResultsAdapter.OnCompleteListener {
    public static final String TAG = WunderbarDevicesActivity.class.getCanonicalName();
    ...
    // Controller
    private WunderbarDevicesController wunderbarDevicesController;
    ...
    protected void onResume() {
        super.onResume();
        wunderbarDevicesController = WunderbarDevicesController.getInstance(this);
        devicesAdapter = new DevicesAdapter(this, this, R.layout.card_device, wunderbarDevicesController.getSubscribedDevicesAsList());
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (RelayrSdk.isBleSupported() && RelayrSdk.isBleAvailable()) {
                    new ScanWunderbarTask(WunderbarDevicesActivity.this).execute(BleDeviceType.WunderbarLIGHT, BleDeviceType.WunderbarGYRO, BleDeviceType.WunderbarHTU, BleDeviceType.WunderbarMIC);
                } else {
                    requestPermission(Manifest.permission.ACCESS_FINE_LOCATION, PERMISSION_REQUEST_ACCESS_FINE_LOCATION);
                }
            }
    ...
    public void onFinished() {
        ...
        if (!wunderbarDevicesController.getScannedDevices().isEmpty()) {

            WunderbarScanResultsDialog dialog = new WunderbarScanResultsDialog();
            Bundle b = new Bundle();
            b.putCharSequence(getResources().getString(R.string.bundle_dialog_title), getResources().getString(R.string.devices));
            dialog.setArguments(b);
            dialog.show(getFragmentManager(), WunderbarScanResultsDialog.TAG);
        } else {
    ...
    public void onReceivedReading(BleDevice device, Reading reading) {
        if (wunderbarDevicesController.getSubscribedDevices().containsKey(device.getAddress())) {
            wunderbarDevicesController.updateSubscribedDevice(device.getAddress(), reading);
        }

        wunderbarDevicesController.getSubscribedDevices().containsKey(device.getAddress());
